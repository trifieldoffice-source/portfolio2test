        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          {plans.map((plan, index) => (
            <AnimationWrapper 
              ...
              animation="fade-up" 
              delay={index * 0.2}
              className="flex flex-col bg-white/[0.03] border border-white/10 hover:border-accent/50 transition-colors duration-500 p-10 relative overflow-hidden group"
            >
              {/* Background Accent */}
              <div className="absolute -right-8 -top-8 w-24 h-24 bg-accent/5 blur-3xl rounded-full group-hover:bg-accent/10 transition-colors duration-500" />
              
              <div className="mb-8">{plan.icon}</div>
              
              <div className="mb-6">
                <span className="text-accent tracking-[0.3em] font-serif text-sm mb-2 block">{plan.name}</span>
                <h3 className="text-2xl font-serif mb-1">{plan.subtitle}</h3>
                <p className="text-accent/80 text-xl font-serif">{plan.price}</p>
              </div>
