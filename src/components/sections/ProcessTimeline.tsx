          ...
          {steps.map((item, index) => (
            <div key={item.id} className={`flex flex-col md:flex-row items-center gap-12 ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                ...
                <AnimationWrapper animation={index % 2 === 0 ? "fade-right" : "fade-left"}>
                  <div className="space-y-6">
                    <div className="space-y-1">
                      <span className="text-accent font-serif text-sm tracking-[0.3em] font-medium">{item.id}</span>
                      <h3 className="text-2xl md:text-3xl font-serif tracking-wide">{item.title}</h3>
                      <p className="text-accent/60 text-[10px] tracking-widest uppercase font-sans">{item.sub}</p>
                    </div>
              ...
              <div className="relative z-10 flex items-center justify-center shrink-0">
                <div className="w-14 h-14 bg-black border border-accent/50 rounded-full flex items-center justify-center text-accent shadow-[0_0_20px_rgba(0,209,255,0.1)] group-hover:shadow-[0_0_30px_rgba(0,209,255,0.2)] transition-shadow duration-500">
                  {item.icon}
                </div>
              </div>
